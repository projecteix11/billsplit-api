...
class TestAuthOnListOrders:
    ...
    def test_bearer_but_empty_token_returns_401(self, client: TestClient):
        # "Bearer " with trailing space but empty token → verify_token will reject
        with patch("app.db.supabase.verify_token", side_effect=ValueError("invalid")):
    ...
    def test_invalid_token_returns_401_with_error_envelope(self, client: TestClient):
        with patch("app.db.supabase.verify_token", side_effect=ValueError("expired")):
    ...
    def test_missing_header_returns_401_with_error_envelope(self, client: TestClient):
        resp = client.get("/api/orders")
        body = resp.json()
        assert resp.status_code == 401
        assert body["data"] is None
...
class TestAuthOnKitchenStatus:
    ...
    def test_invalid_token_returns_401(self, client: TestClient):
        with patch("app.db.supabase.verify_token", side_effect=ValueError("bad")):
            resp = client.patch(
                "/api/order-items/item-1/kitchen-status",
                json={"status": "ready"},
